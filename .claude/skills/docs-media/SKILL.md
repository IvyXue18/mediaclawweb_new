...
Local folder convention: `public/imgs/docs/<slug>/` (create it if this page predates the folder pre-creation pass). Naming convention the user follows when dropping files in:
...
Match files to slots by the leading number. If the file count doesn't match the slot count, or numbering is ambiguous, stop and ask — don't guess a mapping.
